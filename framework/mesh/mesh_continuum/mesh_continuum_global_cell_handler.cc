...
GlobalCellHandler::PushBack(std::unique_ptr<Cell> new_cell)
{
  if (new_cell->partition_id == static_cast<uint64_t>(opensn::mpi_comm.rank()))
  {
    ...
    global_to_native_map_[local_cells_ref_.back()->global_id] = local_cells_ref_.size() - 1;
  }
  else
  {
    ghost_cells_ref_.push_back(std::move(new_cell));
    global_to_foreign_map_[ghost_cells_ref_.back()->global_id] = ghost_cells_ref_.size() - 1;
  }
}
...
{
  auto local_it = global_to_native_map_.find(cell_global_index);
  if (local_it != global_to_native_map_.end())
    return *local_cells_ref_[local_it->second];

  auto ghost_it = global_to_foreign_map_.find(cell_global_index);
  if (ghost_it != global_to_foreign_map_.end())
...
{
  auto foreign_location = global_to_foreign_map_.find(cell_global_index);

  if (foreign_location != global_to_foreign_map_.end())
