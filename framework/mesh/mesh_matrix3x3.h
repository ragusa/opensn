...
struct Matrix3x3
{
  std::array<double, 9> vals{
    0., 0., 0., 0., 0., 0., 0., 0., 0.}; // Row-major storage for matrix elements
  ...
  {
    std::array<double, 3> i_vec = {vec.x, vec.y, vec.z}; // Input vector as std::array
  ...
  {
    vals[i * 3 + j] = value; // Adjusted index calculation
  }
